---
name: receipt-mvp
description: Build the full corporate receipt/expense sorting MVP end-to-end in this repo. Perform 5 internal implement-verify cycles per single agent turn, self-heal errors, and continue until Definition of Done is met, with minimal user prompts.
---
...
5 loops = 1 turn (mandatory internal iteration)
Within a SINGLE agent turn, perform AT LEAST 5 full internal cycles of:

IMPLEMENT → VERIFY → FIX (if needed) → VERIFY

Rules:
- These 5 cycles MUST be executed without yielding control to the user.
- Do NOT pause, summarize, or ask for continuation between cycles.
- Treat the 5 cycles as one atomic unit of work.
- Only after completing 5 cycles, assess the Definition of Done.
- If DoD is NOT satisfied, immediately start another group of 5 cycles within the same turn.

---

Role separation protocol (mandatory)

Role: IMPLEMENTER
Responsibilities:
- Write or modify code/config
- Add/update files
- Fix issues reported by VERIFIER

Rules:
- Do NOT declare completion
- Do NOT perform verification beyond basic sanity

Role: VERIFIER
Responsibilities:
- Run verification commands only
- Check DoD and spec compliance
...
Verification report format (JSON only):
{
  "status": "PASS | FAIL",
  "blocking_issues": [
  ...
  "next_actions_for_implementer": [
    "string"
  ]
}

Notes:
- Reports during the 5 internal cycles are INTERMEDIATE.
- Only ONE FINAL report may conclude execution.
- The agent may stop ONLY after a FINAL report with status=PASS and DoD satisfied.

---

Agent Loop (inside each internal cycle)
For each cycle:
1) IMPLEMENTER implements one small increment
2) VERIFIER runs:
   - npx prisma validate
   - npm run build (preferred) OR npm run dev
3) If FAIL:
   - IMPLEMENTER fixes only blocking issues
   - VERIFIER re-checks
4) Count the cycle as completed
...
6) Continue internal cycles — do NOT stop early

---

Ground rules (non-negotiable)
- Deterministic rules always override LLM results (tax, invoice).
- VendorRule is evaluated BEFORE LLM.
...
- Vendor dictionary rules
- LLM assist only when rules do not hit
- Human review UI
- Excel export optimized for TKC manual input

Excel export (TKC-friendly order)
1. 計上日
2. 勘定科目
3. 摘要
4. 金額（税込）
...
Obey “5 loops = 1 turn”.
Never stop early. Only stop after a FINAL PASS report and DoD is satisfied.
